...
const passportGroupSchema = z.object({
  passportSeries: z
    .string("Серия паспорта обязательна при заполнении")
    .min(1, "Серия паспорта обязательна при заполнении"),
  passportNumber: z
    .string("Номер паспорта обязателен при заполнении")
    .min(1, "Номер паспорта обязателен при заполнении"),
  passportIssuedBy: z.string("Кем выдан обязателен при заполнении").min(1, "Кем выдан обязателен при заполнении"),
  passportDivisionCode: z
    .string("Код подразделения обязателен при заполнении")
    .min(1, "Код подразделения обязателен при заполнении"),
  passportIssueDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Дата выдачи должна быть в формате ГГГГ-ММ-ДД"),
});
