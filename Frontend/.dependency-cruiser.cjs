...
module.exports = {
  forbidden: [
    ...
    {
      name: "fsd-hierarchy-shared",
      severity: "error",
      comment: "The Shared layer cannot import from upper layers (Entities, Features, Widgets, Pages, App).",
      from: { path: "^src/shared" },
      to: { path: "^src/(entities|features|widgets|pages|app)" },
    },

    // Rule: Entities layer cannot import upper layers
    {
      name: "fsd-hierarchy-entities",
      severity: "error",
      comment: "The Entities layer cannot import from upper layers (Features, Widgets, Pages, App).",
      from: { path: "^src/entities" },
      to: { path: "^src/(features|widgets|pages|app)" },
    },

    // Rule: Features layer cannot import upper layers
    {
      name: "fsd-hierarchy-features",
      severity: "error",
      comment: "The Features layer cannot import from upper layers (Widgets, Pages, App).",
      from: { path: "^src/features" },
      to: { path: "^src/(widgets|pages|app)" },
    },

    // Rule: Widgets layer cannot import upper layers
    {
      name: "fsd-hierarchy-widgets",
      severity: "error",
      comment: "The Widgets layer cannot import from upper layers (Pages, App).",
      from: { path: "^src/widgets" },
      to: { path: "^src/(pages|app)" },
    },

    // Rule: Pages layer cannot import App layer
    {
      name: "fsd-hierarchy-pages",
      severity: "error",
      comment: "The Pages layer cannot import from the App layer.",
      from: { path: "^src/pages" },
      to: { path: "^src/app" },
    },

    {
      name: "fsd-slice-isolation",
      severity: "error",
      comment:
        "Cross-slice imports are forbidden. A slice in (Pages, Widgets, Features, Entities) cannot import another slice from the same layer. Communicate via the App layer or composition.",
      from: {
        // Capture the layer ($1) and the slice name ($2)
        path: "^src/(pages|widgets|features|entities)/([^/]+)/.+",
      },
      to: {
        // Match the same layer ($1)
        path: "^src/$1/([^/]+)/.+",
        // But ensure the slice name ($2) is different
        pathNot: "^src/$1/$2/.+",
      },
    },
    {
      name: "fsd-public-api-explicit",
      severity: "error",
      comment: "Разрешен импорт только из Public API (index.ts) другого слайса.",
      from: {
        // Захватываем слой ($1) и слайс ($2)
        path: "^src/([^/]+)/([^/]+)/.+",
      },
      to: {
        path: "^src/([^/]+)/([^/]+)/.+",
        pathNot: [
          // 1. РАЗРЕШАЕМ: внутри одного и того же слайса (самому себе можно всё)
          "^src/$1/$2/.+",

          // 2. РАЗРЕШАЕМ: если путь заканчивается на index.ts/tsx/js/jsx на любом уровне вложенности
          // Это позволит импортировать как '@/features/auth/index.ts',
          // так и '@/features/auth/get-user/index.ts'
          "/index\\.(ts|tsx|js|jsx)$",

          // 3. РАЗРЕШАЕМ: импорты из Shared (там структура может быть плоской без index)
          "^src/shared/.+",
        ],
      },
    },
        ...
        theme: {
          modules: [
            ...
            },
            {
              ...
              attributes: { fillcolor: "#ffa844" },
            },
            {
              criteria: { source: "^src/shared/[^/]+" },
              attributes: { fillcolor: "#d89bf0" },
            },
          ],
          graph: {
            splines: "ortho",
...
// generated: dependency-cruiser@17.3.7 on 2026-01-24T16:07:26.947Z
